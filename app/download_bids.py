...
def download_bids(mrsession_id, session, exam_no):
    ...
    scan_download_url = constants.scan_download_url(mrsession_id)
    ...
    if os.path.exists(zipfile_path):  # Make sure zipfile wasn't already downloaded
        ...
        try:
            ...
            download(session=session, url=scan_download_url, path=zipfile_path, chunk_size=constants.nifti_chunk_size) # download max of 100mb chunks
            print("Download of ", zipfile_path, " complete. \nUnzipping archive...")
            unzip(zipfile_path)
    else:
        try:    # attempt download if the archive is corrupt or incomplete
            download(session=session, url=scan_download_url, path=zipfile_path, chunk_size=constants.nifti_chunk_size) 
