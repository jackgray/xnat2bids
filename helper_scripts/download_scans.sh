...
bash download_scans_sh.sh -d ~/testout -o https://xnat.nyspi.org -i marsonc -s 10000068 -f json -u grayjoh -p taprik-2vacno-naTbag
...
if [[ -n $format ]]; then
    if [[ $format != DICOM && $format != NIFTI && $format != ANALYZE && $format != json ]]; then
        usage
