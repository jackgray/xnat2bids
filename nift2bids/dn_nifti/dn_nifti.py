...
with open (session_list_csv, 'r') as f:
    ...
    for line in lines:
        ...
        csv_exam_no = line.split(',')[-2]
        csv_exam_nos.append(csv_exam_no)
        ...
        if csv_exam_no in download_queue:
            print('Found exam ' + str(csv_exam_no) + '. Downloading...')

            print("Args for job " + str(active_job_no) + ' of ' + str(total_jobs) + ": " )
            print(*args)
            print("\n")

            bidsonly_exam_path = bidsonly_path + csv_exam_no
            ...
            exam_path = bidsonly_path + '/' + csv_exam_no
            zipfile = csv_exam_no + '.zip'
