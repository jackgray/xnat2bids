...
for line in lines:  # a row in session list csv file. each row is one session. 
    ...
    if label == exam:
        ...
        ## GET SESSION INFO AKA SERIES DESCRIPTIONS AND CORRESPONDING NUMBERS
        scan_list_url = str(xnat_url + '/data/experiments/' + mrsession_id + '/scans')
        mrScanData = session.get(scan_list_url)
...
with open(session_list_csv, 'wb') as f:
    ...
    with session.get(session_list_url, stream=True) as r:
        ...
        # WRITE ALL NIFTI INTO ZIPFILE 
        print("Writing NIFTI files first...")
        ...
        series_descriptions = json.loads(mrScanData)
        for i in series_descriptions:
            if 'gre' in i['series_description']
                print("Found " i)
                print("Use " + i['URI'] + ' to download.')
