...
if len(username_password) > 0:
    username_password = username_password\
        .decode("utf-8")
    userpass_array = username_password.split(" ")
    xnat_username = userpass_array[0].strip()    # split created an array deliminated by spaces. #the first item in that array must be the username.
    xnat_password = userpass_array[2].strip()    # second item in index ([1]) could be delimiter. use [-1] to get last element
else:
